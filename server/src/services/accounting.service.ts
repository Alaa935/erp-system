...
export const accountingService = {
  async financialOverview() {
    const [
      ...
      prisma.salesOrder.findMany({ where: { deletedAt: null }, select: { id: true, date: true, orderNumber: true, dueDate: true, totalAmount: true, paidAmount: true, paymentStatus: true, customerId: true, status: true, settledAmount: true, isSettledWithWarehouse: true } }),
    ...
    return {
      ...
      vehicles: vehicles.map(v => ({ id: v.id, name: v.name, plateNumber: v.plateNumber, licenseExpiry: v.licenseExpiry, lastMaintenance: v.lastMaintenance, status: v.status, deletedAt: v.deletedAt, deleteReason: v.deleteReason })),
